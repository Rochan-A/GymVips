...
def async_vips_env(num_episodes, num_envs, dataset, view_sz, max_episode_len):
    ...
    for i in tqdm(range(num_episodes)):
        ...
        assert obs.shape == (
            ...
            3,
            *view_sz,
        ), f"Obs size did not match! Expected {(num_envs, 3, *view_sz)}, got {obs.shape}!"

        while True:
            obs, reward, dones, truncateds, infos = envs.step(
                np.random.rand(num_envs, 2)
            )
            assert obs.shape == (
                num_envs,
                3,
...
def async_cv2_env(num_episodes, num_envs, dataset, view_sz, max_episode_len):
    envs = gym.vector.AsyncVectorEnv(
        [lambda: Cv2Env(dataset, view_sz, max_episode_len) for _ in range(num_envs)]
    )
    ...
    for i in tqdm(range(num_episodes)):
        ...
        while True:
            obs, reward, dones, truncateds, infos = envs.step(
                tuple([(random.random(), random.random()) for _ in range(num_envs)])
            )
            assert obs.shape == (
                num_envs,
                3,
...
if PYVIPS:
    ...
    class PyVipsEnv(gym.Env):
        ...
        def reset(self):
            ...
            img_pth = self.np_random.choice(list(self.files.keys()))
            self.A, self.target = (
                pyvips.Image.new_from_file(img_pth, access="random"),
                self.files[img_pth],
            )
            ...
            crop = np.array(
                self.regs.fetch(
                    up_left[0], up_left[1], self.view_sz[0], self.view_sz[1]
                )
            ).reshape(*self.view_sz, 3)
    ...
    def async_pyvips_env(num_episodes, num_envs, dataset, view_sz, max_episode_len):
        envs = gym.vector.AsyncVectorEnv(
            [
                lambda: PyVipsEnv(dataset, view_sz, max_episode_len)
...
if __name__ == "__main__":
    dataset = {
        "/workspace/rochan/c_envs/000920ad0b612851f8e01bcc880d9b3d.tiff": 1,
        "/workspace/rochan/c_envs/000920ad0b612851f8e01bcc880d9b3d.tiff": 0,
    }
    num_envs = 45
    view_sz = (256, 256)
    max_episode_len = 100
